...
app.post("/api/ai/refine-comm", requireAuth, requirePlan("standard"), async (req, res) => {
  ...
  const client = getGeminiClient();
  if (!client) {
    let refinedText = messageText.trim();
    if (action === "Shorten") {
      refinedText = messageText.trim()
        .replace(/\bplease remember\b/gi, "reminder:")
        .replace(/\bkindly arrange to\b/gi, "please")
        .replace(/\bat your earliest convenience\b/gi, "soon")
        .replace(/\bthank you for your continued support\b/gi, "Thank you!");
      if (refinedText === messageText.trim()) {
        refinedText = `${messageText.trim()} (Thank you for your support.)`;
      }
    } else if (action === "Make Warmer") {
      const salutation = recipientName ? `Dear ${recipientName},\n\n` : `Warm greetings,\n\n`;
      refinedText = `${salutation}${messageText.trim()}\n\nThank you for your kindness and support. God bless you!`;
    } else if (action === "Make More Professional") {
      const salutation = recipientName ? `Dear ${recipientName},\n\n` : `Dear Supporter,\n\n`;
    ...
    return res.json({ 
      message: refinedText, 
      generatedBy: "Local AI Fallback Engine (No active API key)" 
    });
  }

  try {
    const prompt = `You are the HarambeeFlow Treasurer AI Assistant for community and church campaigns in Kenya.
Your task is to refine and improve a message draft written manually by the Treasurer for a supporter (${recipientName || "Supporter"}).
...
Requested Action: ${action} (Options: Improve, Shorten, Make More Professional, Make Warmer)

CRITICAL INSTRUCTIONS & CONSTRAINTS:
1. PRESERVE ALL FACTS EXACTLY: All monetary amounts (e.g. KSh 5,000), dates (e.g. Friday), deadline times, bank/M-PESA paybill numbers, reference codes, names, and specific instructions. You MUST NOT change or invent any numbers, dates, or financial facts.
2. DO NOT INVENT FACTS: Do not invent missing transaction IDs, balances, deadlines, or account numbers.
3. LANGUAGE & TONE: Improve grammar, readability, professional clarity, and respectful community/church tone according to the requested action (${action}). Keep it natural, warm, and clear for Kenyan supporters.
4. FORMATTING: Output ONLY the refined message text. Do not output markdown code blocks, JSON, or explanatory preambles. Output clean, ready-to-use text.`;
    ...
    const text = response.text || messageText;
    return res.json({ message: text.trim(), generatedBy: "HarambeeFlow Treasurer AI" });
  } catch (err: any) {
    console.error("Gemini message refinement failed:", err);
    return res.status(500).json({ error: "Gemini refinement failed", details: err.message });
  }
});
