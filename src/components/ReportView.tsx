...
import { 
  FileText, Download, Printer, Users, Landmark, Target, BarChart3, ChevronRight, Coins, HeartHandshake
} from "lucide-react";
...
export default function ReportView({
  ...
  useEffect(() => {
    if (!db || activeProject.id === "demo-project-id") {
      // Load sample pledges for demo/sandbox mode
      setPledges([
        {
          id: "pledge-1",
        ...
        },
        {
          id: "pledge-2",
          projectId: "demo-project-id",
          donorName: "Mama Fatuma",
          phone: "254700111222",
          email: "fatuma@village.org",
          pledgedAmount: 15000,
          paidAmount: 15000,
          balance: 0,
          status: "Fulfilled",
          ...
          notes: "Given during Sunday service",
          createdAt: new Date().toISOString(),
          updatedAt: new Date().toISOString(),
          paymentHistory: []
        }
    ...
    const unsubscribe = onSnapshot(pledgesRef, (snapshot) => {
      ...
      snapshot.forEach((doc) => {
        ...
        if (data.projectId === activeProject.id) {
          list.push({ id: doc.id, ...data } as Pledge);
        }
      });
      setPledges(list);
    }, (err) => {
      console.error("Failed to sync pledges for report:", err);
    });
  ...
  const totalPledged = useMemo(() => {
    return pledges.reduce((sum, p) => sum + Number(p.pledgedAmount), 0);
  }, [pledges]);

  const totalPledgePaid = useMemo(() => {
    return pledges.reduce((sum, p) => sum + Number(p.paidAmount), 0);
  }, [pledges]);

  const totalPledgeBalance = useMemo(() => {
    return pledges.reduce((sum, p) => sum + Number(p.balance), 0);
  }, [pledges]);

  const pledgeFulfillmentRate = useMemo(() => {
    if (totalPledged === 0) return 0;
    return Math.round((totalPledgePaid / totalPledged) * 100);
  }, [totalPledged, totalPledgePaid]);
    ...
    <div className="flex-1 overflow-y-auto bg-slate-950 p-4 sm:p-6 text-slate-100 min-h-full">
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="space-y-2">
          <p className="text-[10px] font-mono font-bold text-emerald-400 uppercase tracking-widest pl-1">💰 Received Contributions</p>
        ...
        {/* Pledge Metrics Cards Section */}
        <div className="space-y-2">
          <p className="text-[10px] font-mono font-bold text-sky-400 uppercase tracking-widest pl-1">🤝 Promises & Pledges</p>
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-4" id="reports-pledges-cards">
            {/* Pledge Card 1: Total Pledged */}
            <div className="p-5 bg-slate-900/80 backdrop-blur-md border border-slate-800/80 rounded-2xl flex items-center gap-4 shadow-lg shadow-sky-950/5">
              <div className="p-3 bg-sky-500/10 rounded-xl text-sky-400">
                <Coins className="w-5 h-5" />
              </div>
              <div className="space-y-1">
                <span className="text-[11px] font-sans font-bold text-slate-400 uppercase tracking-wider block">Total Pledged</span>
                <p className="text-xl font-black text-sky-400 font-sans">KES {totalPledged.toLocaleString()}</p>
              </div>
            </div>

            {/* Pledge Card 2: Reconciled Pledges */}
            <div className="p-5 bg-slate-900/80 backdrop-blur-md border border-slate-800/80 rounded-2xl flex items-center gap-4 shadow-lg">
              <div className="p-3 bg-[#075e54]/10 rounded-xl text-[#10b981]">
                <HeartHandshake className="w-5 h-5" />
              </div>
              <div className="space-y-1">
                <span className="text-[11px] font-sans font-bold text-slate-400 uppercase tracking-wider block">Pledges Collected</span>
                <p className="text-xl font-black text-emerald-400 font-sans">KES {totalPledgePaid.toLocaleString()}</p>
              </div>
            </div>

            {/* Pledge Card 3: Outstanding Promises */}
            <div className="p-5 bg-slate-900/80 backdrop-blur-md border border-slate-800/80 rounded-2xl flex items-center gap-4 shadow-lg">
              <div className="p-3 bg-amber-500/10 rounded-xl text-amber-400">
                <Coins className="w-5 h-5 text-amber-400" />
              </div>
              <div className="space-y-1">
                <span className="text-[11px] font-sans font-bold text-slate-400 uppercase tracking-wider block">Outstanding Balance</span>
                <p className="text-xl font-black text-amber-500 font-sans">KES {totalPledgeBalance.toLocaleString()}</p>
              </div>
            </div>

            {/* Pledge Card 4: Fulfillment Rate */}
            <div className="p-5 bg-slate-900/80 backdrop-blur-md border border-slate-800/80 rounded-2xl flex items-center gap-4 shadow-lg">
              <div className="p-3 bg-indigo-500/10 rounded-xl text-indigo-400">
                <Target className="w-5 h-5" />
              </div>
              <div className="space-y-1">
                <span className="text-[11px] font-sans font-bold text-slate-400 uppercase tracking-wider block">Fulfillment Rate</span>
                <p className="text-xl font-black text-indigo-400 font-sans">{pledgeFulfillmentRate}%</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 space-y-6 shadow-xl" id="panel-report-preview">
          ...
          <div className="bg-white text-slate-900 p-8 rounded-2xl border border-slate-200 space-y-8 font-sans relative shadow-md">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-8 text-xs border-b border-slate-100 pb-5">
              ...
              <div className="flex justify-between py-1 border-b border-slate-50">
                <span className="text-slate-400 font-medium">Contributions Raised</span>
                <span className="font-extrabold text-emerald-700">KES {totalRaised.toLocaleString()} ({percentComplete}%)</span>
              </div>
              <div className="flex justify-between py-1 border-b border-slate-50">
                <span className="text-slate-400 font-medium">Total Pledges Promised</span>
                <span className="font-extrabold text-sky-700">KES {totalPledged.toLocaleString()}</span>
              </div>
              <div className="flex justify-between py-1 border-b border-slate-50">
                <span className="text-slate-400 font-medium">Pledges Reconciled</span>
                <span className="font-extrabold text-emerald-700">KES {totalPledgePaid.toLocaleString()} ({pledgeFulfillmentRate}%)</span>
              </div>
              <div className="flex justify-between py-1 border-b border-slate-50">
                <span className="text-slate-400 font-medium">Pledges Outstanding</span>
                <span className="font-extrabold text-amber-600">KES {totalPledgeBalance.toLocaleString()}</span>
              </div>
            ...
            {/* Pledge Fulfillment Ledger Table */}
            <div className="space-y-3 pt-4">
              <h5 className="text-xs font-black uppercase tracking-wide text-slate-700">Pledges & Promises Ledger</h5>
              <div className="border border-slate-100 rounded-xl overflow-hidden">
                <table className="w-full text-left text-xs border-collapse">
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-200 text-slate-500 font-bold text-[10px] uppercase tracking-wider">
                  ...
                  <tbody className="divide-y divide-slate-100 font-sans text-xs">
                    {pledges.length === 0 ? (
                      <tr>
                        <td colSpan={6} className="py-8 text-center text-slate-400 italic">No supporter pledges registered for this campaign.</td>
                      </tr>
                    ) : (
                      pledges.map((p, index) => {
                        const date = p.dueDate ? new Date(p.dueDate).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' }) : "Pending";
                        return (
                          <tr key={p.id} className={index % 2 === 0 ? "bg-white" : "bg-slate-50/55"}>
                            <td className="py-3.5 px-4 font-mono text-slate-500 text-[11px]">{date}</td>
                            <td className="py-3.5 px-4 font-black text-slate-800">
                              <div>{p.donorName}</div>
                              <div className="text-[10px] text-slate-400 font-mono">{p.phone}</div>
                            </td>
                            <td className="py-3.5 px-4 font-semibold text-slate-600">KES {Number(p.pledgedAmount).toLocaleString()}</td>
                            <td className="py-3.5 px-4 font-semibold text-emerald-700">KES {Number(p.paidAmount).toLocaleString()}</td>
                            <td className="py-3.5 px-4 font-black text-amber-600">KES {Number(p.balance).toLocaleString()}</td>
                            <td className="py-3.5 px-4 text-right">
                              <span className={`inline-flex px-2 py-0.5 rounded-full text-[9px] font-extrabold uppercase font-mono ${
                                p.status === "Fulfilled" ? "bg-emerald-50 text-emerald-700 border border-emerald-200" :
                                p.status === "Partial" ? "bg-amber-50 text-amber-700 border border-amber-200" :
