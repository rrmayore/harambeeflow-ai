...
export default function PublicCampaignPageView({
  ...
  const [simStep, setSimStep] = useState<"idle" | "pin" | "processing" | "success">("idle");
  ...
  const projContributions = contributions.filter(c => c.projectId === activeProject.id || c.campaignId === activeProject.id);
  ...
  const publicUrl = `${window.location.origin}/#/public/${activeProject.id}`;
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-12 relative z-10">
        ...
        <section className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 relative overflow-hidden text-left shadow-lg">
              <span className="text-slate-400 text-[10px] font-mono block">AMOUNT RAISED</span>
              <p className="text-xl font-black text-white mt-1">KES {raisedAmount.toLocaleString()}</p>
              <div className="flex items-center gap-1 text-[10px] text-emerald-400 font-mono mt-1 font-bold">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 relative overflow-hidden text-left shadow-lg">
              <span className="text-slate-400 text-[10px] font-mono block">GOAL TARGET</span>
              <p className="text-xl font-black text-white mt-1">KES {activeProject.targetAmount.toLocaleString()}</p>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 relative overflow-hidden text-left shadow-lg">
              <span className="text-slate-400 text-[10px] font-mono block">REMAINING BALANCE</span>
              <p className="text-xl font-black text-white mt-1">KES {remainingNeeded.toLocaleString()}</p>
              <span className="text-[10px] text-slate-500 font-mono mt-1.5 block">
                Required milestone deficit
              </span>
            </div>

            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 relative overflow-hidden text-left shadow-lg">
              <span className="text-slate-400 text-[10px] font-mono block">CONTRIBUTOR COUNT</span>
              <p className="text-xl font-black text-white mt-1">{projContributions.length}</p>
              <span className="text-[10px] text-slate-400 font-mono mt-1.5 block font-bold">
                Avg: KES {averageDonation.toLocaleString()}
              </span>
            </div>

            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 relative overflow-hidden text-left shadow-lg">
              <span className="text-slate-400 text-[10px] font-mono block">LARGEST DONATION</span>
              <p className="text-xl font-black text-white mt-1">KES {largestDonation.toLocaleString()}</p>
              <span className="text-[10px] text-slate-500 font-mono mt-1.5 block">
                Single record holder
              </span>
            </div>

          </div>

          {/* Dynamic Velocity panel (Daily and Weekly Progress indicators) */}
          <div className="bg-slate-900/60 border border-slate-800/80 rounded-2xl p-5 sm:p-6 flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="flex items-center gap-3.5">
              <div className="w-10 h-10 rounded-full bg-emerald-500/10 flex items-center justify-center border border-emerald-500/20 text-emerald-400 animate-pulse">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-400 mt-0.5 leading-relaxed font-sans">
                  Real-time computation of rolling momentum cycles on HarambeeFlow AI.
                </p>
              </div>
            </div>

            <div className="grid grid-cols-2 md:grid-cols-3 gap-6 w-full md:w-auto text-xs font-mono">
              <div className="bg-slate-950 p-3.5 rounded-xl border border-slate-800">
                <span className="text-slate-500 block text-[9px] font-bold">DAILY MOMENTUM (24H)</span>
                <span className="text-emerald-400 font-black block mt-1">KES {dailyProgress.toLocaleString()}</span>
              </div>
              <div className="bg-slate-950 p-3.5 rounded-xl border border-slate-800">
                <span className="text-slate-500 block text-[9px] font-bold">WEEKLY VELOCITY (7D)</span>
                <span className="text-sky-400 font-black block mt-1">KES {weeklyProgress.toLocaleString()}</span>
              </div>
              <div className="bg-slate-950 p-3.5 rounded-xl border border-slate-800 col-span-2 md:col-span-1">
                <span className="text-slate-500 block text-[9px] font-bold">REMAINING TARGET GAP</span>
                <span className="text-amber-400 font-black block mt-1">{percent >= 100 ? "FULLY FUNDED!" : `${Math.round(remainingNeeded / activeProject.targetAmount * 100)}% left`}</span>
              </div>
            </div>
          </div>
        </section>
                ...
                <div className="flex-1 py-4 flex flex-col justify-center">
                  ...
                  {simStep === "idle" && (
                    <form onSubmit={handleInitiateSimPayment} className="space-y-3">
                      ...
                      <button
                        type="submit"
                        className="w-full py-3 bg-emerald-500 hover:bg-emerald-400 text-slate-950 text-xs font-mono font-extrabold rounded-xl transition-all uppercase tracking-wider flex items-center justify-center gap-1 shadow-lg cursor-pointer"
                      >
